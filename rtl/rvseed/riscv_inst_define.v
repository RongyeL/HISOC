...
// branch   
`define BRAN_WIDTH 2
`define BRAN_OFF    `BRAN_WIDTH'b00 // no branch
`define BRAN_TYPE_A `BRAN_WIDTH'b01 // beq/bge/bgeu : branch if the zero flag is high.
`define BRAN_TYPE_B `BRAN_WIDTH'b10 // bne/blt/bltu : branch if the zero flag is low.

// jump   
`define JUMP_WIDTH 2
`define JUMP_OFF  `JUMP_WIDTH'b00 // no jump 
`define JUMP_JAL  `JUMP_WIDTH'b01 // jal 
`define JUMP_JALR `JUMP_WIDTH'b10 // jalr
...
// memory or register data select
`define ALU2REG 1'b0
`define MEM2REG 1'b1
...
`define MEM_LHU `MEM_OP_WIDTH'b100 // load half word (unsigned)
`define MEM_SB  `MEM_OP_WIDTH'b101 // store byte 
`define MEM_SH  `MEM_OP_WIDTH'b110 // store half word
`define MEM_SW  `MEM_OP_WIDTH'b111 // store word
